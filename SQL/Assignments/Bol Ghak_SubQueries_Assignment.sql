...
-- Write a query that retrieves the OrderID, CustomerID, and TotalAmount for
-- each order. Additionally, return the number of orders each customer has
-- placed using a subquery in the SELECT clause.
...
SELECT 
    ...
    CustomerID,
    TotalAmount,
    (SELECT COUNT(*) FROM Orders o2 WHERE o2.CustomerID = o1.CustomerID) AS OrderCount
FROM Orders o1;


-- Task 3: Subquery in FROM Clause
-- Write a query that retrieves customers and their total spending using a
...
-- Write a query that retrieves the CustomerID and the total amount they have
-- spent, but only for those customers whose total spending exceeds the
-- average spending across all orders.
